...
class PlayerManager:
    ...
    def init_player(self, p):
        ...
        # listen for LoopStatus changes
        player.connect("loop-status", self.on_loop_status_changed)

        self.manager.manage_player(player)

        # emit initial loop state
        self.write_loop_state(player.props.loop_status)

    def get_players(self):
        return self.manager.props.players

    def write_loop_state(self, state):
        state = str(state)
        icons = {
            "0": "󰑗",
            "2": "󰑖",
    ...
    def on_loop_status_changed(self, player, new_status, _=None):
        self.write_loop_state(new_status)
